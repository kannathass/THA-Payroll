...
#End Region
    Private Sub FrmOverTimeEntry_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            Me.BringToFront()
            Initialize_Columns()
    ...
    Private Sub Initialize_Columns()
        ...
        DgvColumn = DGVDailyPayroll.Columns("FromHours")
        DgvColumn.DisplayIndex = 2
        DgvColumn = DGVDailyPayroll.Columns("ToHours")
        DgvColumn.DisplayIndex = 2
        ...
        Try
            ...
            If DSEmp.Tables("selectEmployeeDailyPayroll").Rows.Count > 0 Then
                For i = 0 To DSEmp.Tables("selectEmployeeDailyPayroll").Rows.Count - 1
                    ...
                    DGVDailyPayroll("OverTime", i).Value = DSEmp.Tables("selectEmployeeDailyPayroll").Rows(i)("OverTimeHrs")
                Next
            End If
        Catch ex As Exception
            MsgBox(Err.Description)
        ...
        Try
            ...
            If DSEmp.Tables(0).Rows.Count > 0 Then
                For i = 0 To DSEmp.Tables(0).Rows.Count - 1
                    ...
                    DGVDailyPayroll("FromHours", i).Value = "00:00"
                    DGVDailyPayroll("ToHours", i).Value = "00:00"
                Next
            End If
            ...
            For i = 0 To DSEmp.Tables(0).Rows.Count - 1
                For j = 0 To DGVDailyPayroll.NewRowIndex - 1
                    If DSEmp.Tables(0).Rows(i)("EmpId") = DGVDailyPayroll("EmployeeCode", j).Value Then
                        DGVDailyPayroll("FromHours", j).Value = "00:00"
                        DGVDailyPayroll("ToHours", j).Value = "00:00"
    ...
    Public Sub FunUpdateEmployeeDailyPayroll()
        Try
            ...
            For i = 0 To DGVDailyPayroll.NewRowIndex - 1
                ...
                If DGVDailyPayroll("OverTime", i).Value <> Nothing Then
                    MyRow("OverTimeHrs") = DGVDailyPayroll("OverTime", i).Value
                Else
                    MyRow("OverTimeHrs") = "00:00"
                End If
                DStemp.Tables("selectEmployeeDailyPayroll").Rows.Add(MyRow)
                MyClassPayroll.FunUpdateEmployeeDailyPayroll(DStemp, 1)
                'End If
            Next
        Catch ex As Exception
            MsgBox(Err.Description)
